...
object SSCSMYA {
  ...
  val selectUploadRadioButton =
    exec(http("SSCSMYA${service}_060_005_SelectUploadOption")
      .post("/additional-evidence")
      .headers(SSCSMYAHeaders.headers_statement)
      .formParam("additional-evidence-option", "statement")
      .formParam("_csrf", "${csrf}")
      .formParam("continue", "")
      .check(CsrfCheck.save)
      .check(status.is(200))
  ...
  val submitEvidence=
    exec(http("SSCSMYA${service}_070_005_SubmitEvidence")
      .post("/additional-evidence/statement")
      .headers(SSCSMYAHeaders.headers_statement)
      .formParam("question-field", "This is my evidence")
      .formParam("_csrf", "${csrf}")
      .formParam("submit", "Submit your statement to the tribunal")
      .check(status.is(200))
    )
      .exec(http("SSCSMYA${service}_070_010_SubmitEvidence_SessionExt")
        .get("/session-extension")
        .headers(SSCSMYAHeaders.headers_20)
        .check(status.in(200,304))
      )
